...
TEST_F(MarshalTest, BasicIntegerTypes) {
    ...
    *m.as_ref().unwrap() << i8_val << i16_val << i32_val << i64_val;
    ...
    *m.as_ref().unwrap() >> i8_out >> i16_out >> i32_out >> i64_out;
...
TEST_F(MarshalTest, UnsignedIntegerTypes) {
    ...
    *m.as_ref().unwrap() << u8_val << u16_val << u32_val << u64_val;
    ...
    *m.as_ref().unwrap() >> u8_out >> u16_out >> u32_out >> u64_out;
...
TEST_F(MarshalTest, VariableLengthIntegers) {
    ...
    *m.as_ref().unwrap() << v32_small << v32_large << v64_small << v64_large;
    
    v32 v32_small_out, v32_large_out;
    v64 v64_small_out, v64_large_out;
    
    *m.as_ref().unwrap() >> v32_small_out >> v32_large_out >> v64_small_out >> v64_large_out;
...
TEST_F(MarshalTest, DoubleValues) {
    ...
    *m.as_ref().unwrap() << d1 << d2 << d3 << d4 << d5;
    
    double d1_out, d2_out, d3_out, d4_out, d5_out;
    *m.as_ref().unwrap() >> d1_out >> d2_out >> d3_out >> d4_out >> d5_out;
...
TEST_F(MarshalTest, StringValues) {
    ...
    *m.as_ref().unwrap() << empty_str << short_str << long_str << unicode_str << special_chars;
    
    std::string empty_out, short_out, long_out, unicode_out, special_out;
    *m.as_ref().unwrap() >> empty_out >> short_out >> long_out >> unicode_out >> special_out;
...
TEST_F(MarshalTest, VectorValues) {
    ...
    *m.as_ref().unwrap() << empty_vec << int_vec << str_vec << nested_vec;
    
    std::vector<int> empty_vec_out, int_vec_out;
    std::vector<std::string> str_vec_out;
    std::vector<std::vector<int>> nested_vec_out;
    
    *m.as_ref().unwrap() >> empty_vec_out >> int_vec_out >> str_vec_out >> nested_vec_out;
...
TEST_F(MarshalTest, ListValues) {
    ...
    *m.as_ref().unwrap() << empty_list << double_list << str_list;
    
    std::list<int> empty_list_out;
    std::list<double> double_list_out;
    std::list<std::string> str_list_out;
    
    *m.as_ref().unwrap() >> empty_list_out >> double_list_out >> str_list_out;
...
TEST_F(MarshalTest, SetValues) {
    ...
    *m.as_ref().unwrap() << empty_set << int_set << str_set;
    
    std::set<int> empty_set_out, int_set_out;
    std::set<std::string> str_set_out;
    
    *m.as_ref().unwrap() >> empty_set_out >> int_set_out >> str_set_out;
...
TEST_F(MarshalTest, MapValues) {
    ...
    *m.as_ref().unwrap() << empty_map << int_str_map << str_vec_map;
    
    std::map<int, std::string> empty_map_out, int_str_map_out;
    std::map<std::string, std::vector<int>> str_vec_map_out;
    
    *m.as_ref().unwrap() >> empty_map_out >> int_str_map_out >> str_vec_map_out;
...
TEST_F(MarshalTest, UnorderedSetValues) {
    ...
    *m.as_ref().unwrap() << empty_uset << int_uset << str_uset;
    
    std::unordered_set<int> empty_uset_out, int_uset_out;
    std::unordered_set<std::string> str_uset_out;
    
    *m.as_ref().unwrap() >> empty_uset_out >> int_uset_out >> str_uset_out;
...
TEST_F(MarshalTest, UnorderedMapValues) {
    ...
    *m.as_ref().unwrap() << empty_umap << int_double_umap << str_int_umap;
    
    std::unordered_map<int, double> empty_umap_out, int_double_umap_out;
    std::unordered_map<std::string, int> str_int_umap_out;
    
    *m.as_ref().unwrap() >> empty_umap_out >> int_double_umap_out >> str_int_umap_out;
...
TEST_F(MarshalTest, ComplexNestedStructures) {
    ...
    *m.as_ref().unwrap() << complex_data;
    
    ComplexType complex_data_out;
    *m.as_ref().unwrap() >> complex_data_out;
...
TEST_F(MarshalTest, LargeDataSets) {
    ...
    *m.as_ref().unwrap() << large_vec;
    
    std::vector<int> large_vec_out;
    *m.as_ref().unwrap() >> large_vec_out;
...
TEST_F(MarshalTest, MixedDataTypes) {
    ...
    *m.as_ref().unwrap() << int_val << str_val << vec_val << map_val;
    ...
    *m.as_ref().unwrap() >> int_out >> str_out >> vec_out >> map_out;
...
TEST_F(MarshalTest, PeekOperation) {
    ...
    *m.as_ref().unwrap() >> read_val1 >> read_val2;
...
TEST_F(MarshalTest, EdgeCasesEmptyCollections) {
    ...
    *m.as_ref().unwrap() << empty_vec << empty_list << empty_set << empty_map;
    ...
    *m.as_ref().unwrap() >> vec_out >> list_out >> set_out >> map_out;
...
TEST_F(MarshalTest, SpecialFloatingPointValues) {
    ...
    *m.as_ref().unwrap() << inf_pos << inf_neg << nan_val << denorm;
    
    double inf_pos_out, inf_neg_out, nan_val_out, denorm_out;
    *m.as_ref().unwrap() >> inf_pos_out >> inf_neg_out >> nan_val_out >> denorm_out;
...
TEST_F(MarshalTest, RandomizedStressTest) {
    ...
    for (int i = 0; i < num_items; ++i) {
        ...
        *m.as_ref().unwrap() << int_val << double_val << str_val;
    }
    ...
    for (int i = 0; i < num_items; ++i) {
        ...
        *m.as_ref().unwrap() >> int_out >> double_out >> str_out;
