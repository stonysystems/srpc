...
def verify_alpha_service_block(block: str) -> None:
    ...
    assert_contains(
        ...
        "            rrr::MarshalSource __req_src__(&req->m);\n"
        "            rrr::BinaryReadArchive __req_ar__(rrr::make_source_proxy(&__req_src__));\n"
        "            __req_ar__ >> __typed_req__.left;\n"
        "            __req_ar__ >> __typed_req__.right;\n"
    ...
    assert_contains(
        ...
        "            rrr::MarshalSource __req_src__(&req->m);\n"
        "            rrr::BinaryReadArchive __req_ar__(rrr::make_source_proxy(&__req_src__));\n"
        "            __req_ar__ >> __typed_req__.delay_ms;\n"
...
def verify_beta_service_block(block: str) -> None:
    ...
    assert_contains(
        ...
        "            rrr::MarshalSource __req_src__(&req->m);\n"
        "            rrr::BinaryReadArchive __req_ar__(rrr::make_source_proxy(&__req_src__));\n"
        "            __req_ar__ >> __typed_req__.other_id;\n"
        "            auto __typed_result__ = this->ping(__typed_req__);\n"
        "            auto sconn_opt = weak_sconn.upgrade();\n"
        "            if (sconn_opt.is_some()) {\n"
        "                auto sconn = sconn_opt.unwrap();\n"
...
def verify_beta_proxy_block(block: str) -> None:
    ...
    assert_contains(
        ...
        "            rrr::MarshalSource __reply_src__(&*__reply_guard__);\n"
        "            rrr::BinaryReadArchive __reply_ar__(rrr::make_source_proxy(&__reply_src__));\n"
        "            __reply_ar__ >> __typed_resp__.echoed;\n"
        "            return rusty::Result<RpcPingResponse, rrr::i32>::Ok(__typed_resp__);\n"
        "        }\n"
        "        auto operator co_await() const {\n"
        "            return rrr::make_typed_future_awaitable(*this);\n"
