...
// `fiber_yield_invoke(*yield_ptr)`. The DSL `fn new(task)` factory lowers to
// `static new_()`; fiber_install_task calls it only after allocating the task
// in its final Box.
#if RUSTYCPP_RUST
...
// DSL-support alias: the grammar cannot spell fn-type template args. It must
// precede fiber_task_t now that the holder itself is authored as DSL.
using FiberTaskFn = rusty::Function<void(fiber_yield_t&)>;

// @unsafe { raw fiber_task_t* deref + C-engine yield. } Free function,
// authored as DSL further down (§7.30).
void fiber_yield_invoke(fiber_yield_t& self);

// Definitions are generated by the fiber-engine DSL block near the original
// runtime implementation site. These declarations also let fiber_task_t's
// generated Drop and the post-emplacement install path call that boundary.
...
// Thin pinned holder over the plain-C srpc_fiber engine. The mmap stacks,
// ABI context seeding, TLS active slot, and resume/yield/finish state machine
// all live in srpc_fiber.c. Construction is deliberately pure: only after a
// Box gives the holder its stable address does fiber_install_task seed yield_'s
// back-pointer and enter the C engine. PhantomPinned makes that stable-address
// contract explicit and lowers to deleted C++ move operations.
#if RUSTYCPP_RUST
#[cfg_attr(any(), cpp_no_fieldwise_ctor)]
struct fiber_task_t {
    fn_: FiberTaskFn,
    yield_: fiber_yield_t,
    fib_: srpc_fiber,
...
impl fiber_task_t {
    ...
    fn new(fn_: FiberTaskFn) -> fiber_task_t {
        fiber_task_t {
            fn_: fn_,
...
#endif
/*RUSTYCPP:GEN-BEGIN id=reactor.fiber_task version=1 rust_sha256=9da3eb7e5b429a03daaabd2d753bad4e8f57f4de7017ab3d18cdf80f52506adb*/
struct fiber_task_t;
...
struct fiber_task_t {
    ...
    void rusty_mark_forgotten() const noexcept { _rusty_forgotten = true; rusty::detail::mark_forgotten_if_supported(this->fn_); rusty::detail::mark_forgotten_if_supported(this->yield_); rusty::detail::mark_forgotten_if_supported(this->fib_); rusty::detail::mark_forgotten_if_supported(this->_pin); }


    explicit fiber_task_t(FiberTaskFn fn_);
    ~fiber_task_t() noexcept(true);
};


fiber_task_t::fiber_task_t(FiberTaskFn fn_)
    : fn_(std::move(fn_))
    , yield_(fiber_yield_t{.task_ = rusty::ptr::null_mut()})
    , fib_(rusty::MaybeUninit<srpc_fiber>::zeroed().assume_init())
    , _pin(rusty::marker::PhantomPinned{})
{}

fiber_task_t::~fiber_task_t() noexcept(true) {
    if (_rusty_forgotten) { return; }
    fiber_engine_destroy(&this->fib_);
}
/*RUSTYCPP:GEN-END id=reactor.fiber_task*/
 ...
 * through the `fiber_engine_*` DSL functions in the impl section of this
 * file. Those callers carry `// @unsafe` annotations.
 *
...
// DSL-support alias: the grammar cannot spell fn-type template args.
...
fn fiber_install_task(t: *const rusty::RefCell<rusty::Option<rusty::Box<fiber_task_t>>>,
    ...
    let mut boxed = rusty::make_box::<fiber_task_t>(task);
    ...
    let mut installed = rusty::Some(boxed);
    let mut g = (*t).borrow_mut();
    *g = installed;
}
#endif
/*RUSTYCPP:GEN-BEGIN id=reactor.68 version=1 rust_sha256=098b3e51c608726bd2601a171c5a050cffafaf56cdc5154d27befc7518301697*/
void fiber_install_task(const rusty::RefCell<rusty::Option<rusty::Box<fiber_task_t>>>* t, FiberTaskFn task) {
    auto boxed = rusty::make_box<fiber_task_t>(std::move(task));
    fiber_task_t& task_ref = boxed.as_mut();
    fiber_yield_t yield_value = fiber_yield_t::new_(task_ref);
    task_ref.yield_ = std::move(yield_value);
    fiber_task_t* const task_ptr = static_cast<fiber_task_t*>(rusty::detail::ptr_or_addr(task_ref));
    rusty::ffi::c_void* const entry_arg = const_cast<rusty::ffi::c_void*>(reinterpret_cast<const rusty::ffi::c_void*>(task_ptr));
    fiber_engine_start(&task_ref.fib_, entry_arg);
    auto installed = rusty::Some(std::move(boxed));
    auto g = ((*t)).borrow_mut();
    *g = std::move(installed);
}
...
#endif
/*RUSTYCPP:GEN-BEGIN id=reactor.69 version=1 rust_sha256=977002bac443578dc0a7dfa577fdcfbc55a57bf5d9d6a79c5685904cf29c6939*/
...
void fiber_task_invoke(const rusty::RefCell<rusty::Option<rusty::Box<fiber_task_t>>>* t) {
    ...
    fiber_engine_resume(rusty::addr_of_temp_mut((rusty::detail::deref_if_pointer_like(bx)).fib_));
}
...
// Start the fiber: install the task closure. fiber_install_task first gives
// fiber_task_t its stable Box address, then the C engine's initial resume runs
// the body immediately up to the first yield (matching Boost pull_type).
#if RUSTYCPP_RUST
...
#endif
/*RUSTYCPP:GEN-BEGIN id=reactor.fiber_run version=1 rust_sha256=fa9fc81b67543e30893125604853ba5ab98b185ac45b7f2df52eaeb2fc214d14*/
void fiber_run(const Fiber& fb) {
    {
...
{
    {
        auto&& tguard = rusty::borrow((*self_ptr).fiber_task_);
        verify(((rusty::detail::deref_if_pointer_like(tguard))).is_none());
    }
...
// `self` becomes a receiver and would emit a method rather than this free
// function.
//
...
#endif
/*RUSTYCPP:GEN-BEGIN id=reactor.23 version=1 rust_sha256=39677765474a88b742ef955687d5810686f2fd406750ea1287555f7e064a153c*/
...
void fiber_yield_invoke(fiber_yield_t& y) {
    ...
    // @unsafe
    {
        fiber_engine_yield(rusty::addr_of_temp_mut((rusty::detail::deref_if_pointer_like(y.task_)).fib_));
    }
}
...
// Function/yield pair), and the TU-local fiber_task_t exposes precisely
// those fields to these free kernels. `f as bool`
...
#endif
/*RUSTYCPP:GEN-BEGIN id=reactor.87 version=1 rust_sha256=38b4f81a4837b9e9600d7c95d953a47cf3618d82cc362244920c7982bd7256c9*/
...
extern "C" void fiber_task_entry_thunk(rusty::ffi::c_void* arg) {
    ...
    // @unsafe
    {
        fiber_task_body_invoke(rusty::detail::deref_if_pointer_like((*task).fn_), (*task).yield_);
    }
}
