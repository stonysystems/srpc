...
// machine all live in srpc_fiber.c. Only construction/destruction remain
// members; the DSL free functions below resume/yield/invoke through this
// TU-local holder's deliberately exposed storage.
class fiber_task_t {
 public:
  ...
  // The type is defined only in this TU. These are public so the inline-Rust
  // free-function kernels can cross the C boundary without C++ member shims.
  TaskFn fn_;
...
// runtime implementation site; earlier DSL helpers call them directly.
void fiber_engine_resume(srpc_fiber* fib);
void fiber_engine_yield(srpc_fiber* fib);
void fiber_task_body_invoke(fiber_task_t::TaskFn& f, fiber_yield_t& y);
